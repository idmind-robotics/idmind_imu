...
1. Install [BrickDaemon](https://www.tinkerforge.com/en/doc/Software/Brickd.html#brickd) — it
   proxies between USB and the TCP port the driver connects to.
2. Install the Python bindings:
   ```bash
   sudo apt install python3-tinkerforge
   ```
   > `python3-tinkerforge` has **no rosdep rule**, so it is deliberately not declared in
   > `package.xml` — declaring it would make `rosdep install` fail. Install it manually.
...
1. Create `idmind_imu/drivers/<your_imu>.py` with a class deriving from `ImuDriver`
   (`idmind_imu/drivers/base.py`) and implementing `start()`, `stop()`, `apply_config()` and
   `state()`.
2. Convert readings to SI units and report them by calling `on_sample(ImuSample(...))`.
   Put reusable unit maths in `idmind_imu/conversions.py` and unit test it there.
3. Register the class in `idmind_imu/drivers/registry.py`.
...
The suite runs with **no hardware and no BrickDaemon**: `test/fake_brickd.py` implements the
TinkerForge wire protocol as a fake daemon on an ephemeral port, so enumeration, getters,
setters and the sensor callback path are all exercised against the real `tinkerforge`
bindings. Alongside it are pure unit tests for the conversions and integration tests that
spin the real node and assert on published messages.
