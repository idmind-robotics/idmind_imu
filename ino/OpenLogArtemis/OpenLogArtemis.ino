...
bool output = false; // Auxiliary flag to print output to SerialPort
...
void loop()
{
  ...
  if (Serial.available() > 0) {    
    ...
    if(incomingByte == 'f')
    {
      SERIAL_PORT.println(F("IDMind OpenLog Artemis"));
    }
    if(incomingByte == 'r')
    {
      output = true;
  ...
  if(( myICM.status == ICM_20948_Stat_Ok ) || ( myICM.status == ICM_20948_Stat_FIFOMoreDataAvail )) // Was valid data available?
  {
    ...
    //////////////////////////////////////
    // ADDED BY CARLOS NEVES @IDMIND    //
    //////////////////////////////////////
    if ( (data.header & DMP_header_bitmap_Gyro) > 0 )
    { 
      // Scaling is still required
      double wx = (double)data.Raw_Gyro.Data.X * 3.14/180.0; // Extract the raw gyro data, comes in DPS
      double wy = (double)data.Raw_Gyro.Data.Y * 3.14/180.0;
      double wz = (double)data.Raw_Gyro.Data.Z * 3.14/180.0;
      if(output){
        SERIAL_PORT.print(F(" WX:"));
        SERIAL_PORT.print(wx, 3);
        SERIAL_PORT.print(F(" WY:"));
        SERIAL_PORT.print(wy, 3);
        SERIAL_PORT.print(F(" WZ:"));
        SERIAL_PORT.print(wz, 3);
      }
    ...
    {
      float acc_x = (float)data.Raw_Accel.Data.X * 9.82/1000.0; // Extract the raw accelerometer data, comes in miliG
      float acc_y = (float)data.Raw_Accel.Data.Y * 9.82/1000.0;
      float acc_z = (float)data.Raw_Accel.Data.Z * 9.82/1000.0;
      if(output){
    ...
    if(output) 
      SERIAL_PORT.println();
    // This is here because data is not always available, so it remembers a request was made until a reply is given
    output = false;
  }
